  ...
  bump(dryRun = false, tag = false) {
    ...
    this.versions.forEach((v, k) => {
      ...
      if (matches && matches.length === 6) {
        ...
        version = Bump.bumpVersion(matches, options);
        // console.debug(`${k}.version: ${version}`);
        ...
        // console.info(`Bumped ${k} to ${version}`);
        modified = true;
      }
    });
    ...
    if (tag) {
      console.log('trigger git tag', `v${version}`);
